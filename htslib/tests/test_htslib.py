...
class TestHtslibBgzipSomatic:
    ...
    def test_htslib_bgzip_somatic(self):
        ...
        cmd = ['nextflow', 'test_htslib.nf', '-entry', 'test_htslib_bgzip_somatic', '--pwd',  os.getcwd(), '-config', 'test_htslib.config', '--no-resume']
        subprocess.run(cmd)

        expected_output = pjoin(os.getcwd(), 'null', 'test_dataset', 'test_patient', 'test_norm_run_test_tum_run', 'htslib_bgzip', 'test.vcf.gz')
...
class TestHtslibBgzip:
    ...
    def test_htslib_bgzip(self):
        ...
        cmd = ['nextflow', 'test_htslib.nf', '-entry', 'test_htslib_bgzip', '--pwd',  os.getcwd(), '-config', 'test_htslib.config', '--no-resume']
        subprocess.run(cmd)

        expected_output = pjoin(os.getcwd(), 'null', 'test_dataset', 'test_patient', 'test_run', 'htslib_bgzip', 'test.vcf.gz')
...
class TestHtslibTabix:
    ...
    def test_htslib_tabix(self):
        ...
        cmd = ['nextflow', 'test_htslib.nf', '-entry', 'test_htslib_tabix', '--pwd',  os.getcwd(), '-config', 'test_htslib.config', '--no-resume']
        subprocess.run(cmd)
